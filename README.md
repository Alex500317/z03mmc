...
5. Оптимизировано потребление при потере отключении координатора или потере связи.

6. Без регистрации в сети Zigbee термометр не работает. Регистрации производится по контакту “reset”  с gnd или при первом включении питания. Время регистрации для экономии питания ограничено путем отключения работы термометра через 45 сек. Потребление после отключения около 3 мкА.

7. Требуется дополнения-изменения кода для работы с Xiaomi Gateway 3.
